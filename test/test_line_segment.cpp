...
class LineSegmentTestSuite : public ::testing::Test {
 protected:
  void SetUp() override {
    endpoint1 = Geometry::Point(4.0, 11.0);
    endpoint2 = Geometry::Point(5.0, 5.0);
  ...
  Geometry::Point endpoint1;
  Geometry::Point endpoint2;
...
TEST_F(LineSegmentTestSuite, Support) {
  Geometry::Point direction1 = Geometry::Point(1.0, 1.0);
  ...
  Geometry::Point direction2 = Geometry::Point(1, -1.0);
...
TEST_F(LineSegmentTestSuite, contains) {
  ...
  ASSERT_TRUE(line_segment.contains(Geometry::Point(4.5, 8.0)));
  ASSERT_FALSE(line_segment.contains(Geometry::kOrigin));
  ASSERT_FALSE(line_segment.contains(Geometry::Point(4, 7.0)));
  ASSERT_FALSE(line_segment.contains(Geometry::Point(4, 10.0)));
}
...
TEST_F(LineSegmentTestSuite, distance_to) {
  ...
  ASSERT_NEAR(line_segment.distance_to(Geometry::Point(4.5, 8.0)), 0.0, kEPS);

  ASSERT_NEAR(line_segment.distance_to(Geometry::kOrigin), 7.07106781, kEPS);
  ASSERT_NEAR(line_segment.distance_to(Geometry::Point(3.0, 7.0)), 1.643989873,
              kEPS);
}